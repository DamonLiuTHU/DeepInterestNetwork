...
class Model(object):
    def __init__(self, user_count, item_count, cate_count, cate_list):
        ...
        d_layer_wide_j = tf.concat([tf.gather(u_emb, [0], axis=-1) * tf.gather(j_emb, [0], axis=-1),
                                    tf.gather(u_emb, [-1], axis=-1) * tf.gather(j_emb, [-1], axis=-1),
                                    tf.gather(u_emb, [hidden_units // 2], axis=-1) * tf.gather(j_emb,
                                                                                               [hidden_units // 2],
                                                                                               axis=-1)], axis=-1)
        ...
        d_layer_wide_all = tf.concat([tf.gather(u_emb_all, [0], axis=-1) * tf.gather(all_emb, [0], axis=-1),
                                      tf.gather(u_emb_all, [-1], axis=-1) * tf.gather(all_emb, [-1], axis=-1),
                                      tf.gather(u_emb_all, [hidden_units // 2], axis=-1) * tf.gather(all_emb, [
                                          hidden_units // 2], axis=-1)], axis=-1)
