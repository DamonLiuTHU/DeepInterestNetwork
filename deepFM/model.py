...
class Model(object):
    def __init__(self, user_count, item_count, cate_count, cate_list):
        ...
        # fm part
        d_layer_fm_i = tf.concat([tf.reduce_sum(u_emb * i_emb, axis=-1, keep_dims=True),
        ...
        d_layer_fm_j = tf.concat([tf.reduce_sum(u_emb * j_emb, axis=-1, keep_dims=True),
                                  tf.gather(u_emb, [0], axis=-1) + tf.gather(j_emb, [0], axis=-1)], axis=-1)
        ...
        d_layer_fm_all = tf.concat([tf.reduce_sum(u_emb_all * all_emb, axis=-1, keep_dims=True),
                                    tf.gather(u_emb_all, [0], axis=-1) + tf.gather(all_emb, [0], axis=-1)], axis=-1)
        ...
        print
        self.p_and_n.get_shape().as_list()
